...
class EventBus {
  ...
  off(event, callback) {
    if (!this.listeners[event]) {
      // если нет такого eventName то ошибка
      throw new Error(`Нет события: ${event}`)
    }

    this.listeners[event] = this.listeners[event].filter(
      // берем нужный event и фильтруем и убираем нужный обработчик
  ...
  emit(event, ...args) {
    ...
    this.listeners[event].forEach((listener) => {
      //  мы берем из обьекта listeners нужный event и выполняем обработчики
      listener(...args)
    })
  }
}
